...
- **模型目录** —— 28 家厂商、62 个模型；端点、协议、推理方言都是数据。限额和价格由应用自己提供。
...
- **会话** —— 记录 agent 做过什么，再从中恢复对话，你自己的事件也进同一条日志。
- **MCP** —— MCP 服务器的工具直接变成 agent 自己的工具，`WithTools` 直接收。
...
一次模型调用，六种协议任选。这一半的所有内容都不依赖 `pkg/agent`。
...
```sh
go get github.com/genai-io/sdk-go@v0.6.0
```
