...
**不重试。** 重试、缓存、日志、成本计量都是 `Middleware`（`ai.Wrap(driver, retry, meter)`，一个装饰 Driver 的 Driver），因为只有应用知道一轮的预算、什么可以缓存、什么绝不能记日志。有一条规则不该让调用方自己去发现：**重试只能重放一个在产出任何 delta 之前就失败的调用**。

**不压缩。** 判断一段对话太长、总结它、丢掉最老的几轮，都是应用层拿着这个包没有的信息去做的决定。它做的是**修复**：`RepairHistory` 只删协议本身会拒绝的东西——Ctrl-C 留下的没人应答的工具调用、经过 JavaScript 运行时的对话带来的非法 UTF-8。**修复不是策略。**
...
```
  messages + options
        ...
        |  | RepairHistory      配对工具调用，替换非法 UTF-8                |
